"""Filesystem Outbound Adapter implementing SourceProviderPort for PHP."""
...
class FileSourceProvider(SourceProviderPort):
    """Fetches PHP source code files from the local filesystem."""

    def get_sources(self, path: str, extensions: list[str] | None = None) -> dict[str, str]:
        target = Path(path)
        valid_exts = set(extensions) if extensions else {".php", ".phtml", ".inc"}
    ...
    def _scan_directory(self, target: Path, valid_exts: set[str]) -> dict[str, str]:
        ...
        for root, dirs, files in os.walk(target_str):
            # In-place directory pruning to avoid traversing ignored subtrees (vendor, .git, etc.)
